...
Unit Testing (JUnit 5) Tests cover ViewModel logic and repository data handling in isolation, ensuring business rules behave correctly independent of the UI.

UI Testing (Espresso) End-to-end user flows — including student login and item listing submission — are automated to catch regressions early.

Sensor Simulation Because physical hardware cannot be controlled during automated tests, sensor inputs are simulated programmatically:
...
Connect your Firebase project and place the google-services.json file in the /app directory.
Build and run on a physical device or emulator (API 26+).
